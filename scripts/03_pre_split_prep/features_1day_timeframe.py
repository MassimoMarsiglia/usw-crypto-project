...
# save csv
csv_BTC_1day = os.path.join(DATA_DIR, "03_btc_features_1day.csv")

# save images
btc_ema_1day_trend = os.path.join(IMAGE_DIR, "03_BTC_EMA_1Day_Trend.png")
btc_rsi_1day_dist = os.path.join(IMAGE_DIR, "03_BTC_RSI_1Day_Distribution.png")
btc_target_1day_balance = os.path.join(IMAGE_DIR, "03_BTC_Target_1Day_Balance.png")
...
selected_file = csv_USDTpairs_1day # change to another timeframe if needed
...
df_features.to_csv(csv_BTC_1day, index=True)
...
fig1 = plot_df.plot(
    figsize=(14, 6), 
    title='BTC/USDT Trend: Close Price vs. EMAs (1-Day Timeframe)'
).get_figure()
fig1.autofmt_xdate()
fig1.savefig(btc_ema_1day_trend, bbox_inches='tight', dpi=300)
plt.close(fig1)
...
ax2.set_title('RSI(14) Distribution (1-Day Timeframe)')
ax2.set_xlabel('RSI Value')
ax2.set_ylabel('Frequency')
fig2.savefig(btc_rsi_1day_dist, bbox_inches='tight', dpi=300)
plt.close(fig2) 

# Plot target distribution
fig3, ax3 = plt.subplots(figsize=(6, 4))
df_features['target'].value_counts().plot(kind='bar', rot=0, ax=ax3)
ax3.set_title('Target Class Balance 1-Day Timeframe (0: Down/Flat, 1: Up)')
ax3.set_xlabel('Direction')
ax3.set_ylabel('Count')
fig3.savefig(btc_target_1day_balance, bbox_inches='tight', dpi=300)
plt.close(fig3)
