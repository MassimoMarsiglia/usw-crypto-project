...
# save csv
csv_BTC_5mins = os.path.join(LARGE_DATA_DIR, "03_btc_features_5mins.csv")

# save images
btc_ema_5mins_trend = os.path.join(IMAGE_DIR, "03_BTC_EMA_5mins_Trend.png")
btc_rsi_5mins_dist = os.path.join(IMAGE_DIR, "03_BTC_RSI_5mins_Distribution.png")
btc_target_5mins_balance = os.path.join(IMAGE_DIR, "03_BTC_Target_5mins_Balance.png")
...
selected_file = csv_USDTpairs_5mins # change to another timeframe if needed
...
df_features.to_csv(csv_BTC_5mins, index=True)
...
fig1 = plot_df.plot(
    figsize=(14, 6), 
    title='BTC/USDT Trend: Close Price vs. EMAs (5Mins Timeframe)'
).get_figure()
fig1.autofmt_xdate(rotation=45)
fig1.savefig(btc_ema_5mins_trend, bbox_inches='tight', dpi=300)
plt.close(fig1)
...
ax2.set_title('RSI(14) Distribution (5Mins Timeframe)')
ax2.set_xlabel('RSI Value')
ax2.set_ylabel('Frequency')
fig2.savefig(btc_rsi_5mins_dist, bbox_inches='tight', dpi=300)
plt.close(fig2) 

# Plot target distribution
fig3, ax3 = plt.subplots(figsize=(6, 4))
df_features['target'].value_counts().plot(kind='bar', rot=0, ax=ax3)
ax3.set_title('Target Class Balance 5Mins Timeframe (0: Down/Flat, 1: Up)')
ax3.set_xlabel('Direction')
ax3.set_ylabel('Count')
fig3.savefig(btc_target_5mins_balance, bbox_inches='tight', dpi=300)
plt.close(fig3)
