"""
run_simulation_multi.py

Run multi-asset trading simulation with portfolio diversification.
"""
...
from trading_agent import CryptoMLP
...
def main():
    """Run the multi-asset trading simulation."""
    ...
    # Your model predicts: [BTC_4h, BTC_1d, ETH_4h, ETH_1d, LTC_4h, LTC_1d, SOL_4h, SOL_1d]
    TARGET_INDICES = {
    ...
    # CRITICAL: Model was trained on ALL 7 pairs (3 BTC + 4 USDT)
    # We must compute features for all 7 pairs even though we only trade 4 USDT pairs
    ALL_TRAINING_PAIRS = ["ETHBTC", "LTCBTC", "SOLBTC", "BTCUSDT", "ETHUSDT", "LTCUSDT", "SOLUSDT"]
    feature_calculator = create_deployment_feature_calculator(pairs=ALL_TRAINING_PAIRS)
    
    print(f"\nDeployment feature calculator initialized:")
    print(f"  Trading pairs: {ASSETS}")
    print(f"  Feature pairs (must match training): {ALL_TRAINING_PAIRS}")
    print(f"  Timeframes: 15m, 1h, 4h, 1d (resampled from 15m buffer)")
    print(f"  Features per asset-timeframe: 19")
    print(f"  Total expected features: {len(ALL_TRAINING_PAIRS)} × 4 × 19 = {len(ALL_TRAINING_PAIRS) * 4 * 19}")
    ...
    # Run backtest (using adapted version for multi-asset)
    print(f"\n{'='*70}")
    print("Running Multi-Asset Trading Simulation with Alpaca Fee Structure")
    print(f"{'='*70}")
    
    # Setup TensorBoard
    tensorboard_dir = os.path.join(BASE_DIR, "../../runs/eval_feedforward_multi")
    ...
    # Open trade log file
    trade_log_file = os.path.join(BASE_DIR, "../../results/simulation/detailed_trade_log.txt")
    os.makedirs(os.path.dirname(trade_log_file), exist_ok=True)
    trade_log = open(trade_log_file, 'w')
    trade_log.write("="*100 + "\n")
    trade_log.write("DETAILED TRADE LOG - Multi-Asset Simulation\n")
    trade_log.write("="*100 + "\n\n")
    ...
    print(f"\n{'='*60}")
    print("MULTI-ASSET TRADING SIMULATION RESULTS")
    print(f"{'='*60}")
    ...
    print(f"\n✓ Multi-asset simulation complete!")
